...
public class DeveloperController {
    ...
    public DeveloperController(Taxable taxable) {
        this.taxable=taxable; // neyi inject ettim? Taxable interfaceyken bunu kullanmamın anlamı yok ki?
    }

    @PostConstruct
    public void init() {
        this.developers = new HashMap<>();
        this.developers.put(1, new Developer(1, "Emre", 1000d, Experience.JUNIOR));
    }
    ...
    public DeveloperResponse save(@RequestBody Developer developer) {
        ...
        return new DeveloperResponse(createdDeveloper, HttpStatus.CREATED.value(),"create işlemi başlatıldı."); // yeni bir obje dönüyoruz neden? Neden mevcut developer değil; endpointte create işlemi yapıldığında developer objesiyle beraber bir status mesajı dönsün
    }
